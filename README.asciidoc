...
| 给定一个 n(<=50) 个点的凸多边形，顶点是哨塔，按照给定的顺序沿多边形巡逻。现在
想减少巡逻距离，可以选择不经过若干哨塔。但是给定 m(<=1000) 个纪念碑，要求纪念碑
都在巡逻路线内部，求最短的巡逻路线。
...
| ural1097 | {y}
| discretization
| 给定一个 l*l 的正方形，里面有 m(<=100) 个不重叠子正方形，每个正方形都有个权重，
...
个半平面，然后求交就行。注意还有 x>=0, y>=0, x+y<=1。
|====
